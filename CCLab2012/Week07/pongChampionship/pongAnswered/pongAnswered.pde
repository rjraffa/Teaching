/* OpenProcessing Tweak of *@*http://www.openprocessing.org/sketch/5530*@* */
...
//reset variables and shape locations, redraw background
void restartGame() {
  if(keyPressed) {
    if (myDigitalValue[2] == 1) {
      background(200);
      rx = 15;
      ry = 195;
      rb = 195;
...
void rightPaddle() {
  //moving paddle up -> digital pin 2
  ...
  //move paddle down -> digital pin 3
  if(myDigitalValue[1] == 1) {
      lastry = ry;
      ry=ry+2;//paddle velocity
...
void leftPaddle() {
  ...
  rb = int(map(myAnalogValue[0], 0, 944, 30, height-52));

}

//restart ball after a miss
void launchRight() {
  if(myDigitalValue[2] == 1) {
      x = width-25; 
      y = rb+25; 
