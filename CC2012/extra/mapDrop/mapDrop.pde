...
void draw() {
  ...
  for (int i = 0; i < dropPosX.length; i++) {
   ellipse(dropPosX[i], dropPosY[i], 10,10); 
  }
  ...
  if (keyPressed == true) {
      if (key == CODED) {
        if (keyCode == UP) {
          if(millis()%500 <= 250) { 
            image(walkUpA,characterPos.x, characterPos.y);
            if (pawUpOne == true) {
               dropPosX = append(dropPosX, characterPos.x);
               dropPosY = append(dropPosY, characterPos.y);
               pawUpOne = false;
               pawUpTwo = true;
            }
          } else { 
            image(walkUpB,characterPos.x, characterPos.y);
            if (pawUpTwo == true) {
              dropPosX = append(dropPosX, characterPos.x);
              dropPosY = append(dropPosY, characterPos.y);
              pawUpTwo = false;
